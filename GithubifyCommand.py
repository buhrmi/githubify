...
class Githubify(sublime_plugin.TextCommand):
    ...
    def gather_info(self, edit):
        ...
        git_root = self.check_output("git rev-parse --show-toplevel", cwd).strip().decode('utf-8')
        github_repo = self.check_output("git remote -v 2> /dev/null | grep @github | awk '{ print $2 }' | sed -e 's/git@github.com:\\(.*\\).git/\\1/' | head -n 1", cwd).strip().decode('utf-8')
        branch = self.check_output("git branch | sed -e '/^[^*]/d' -e 's/* \\(.*\)/\\1/' -e 's/#/%23/'", cwd).strip().decode('utf-8')
